...
def extract_features(state, env):
    ...
    # Check if moving east is allowed (east movement constraints)
    ...
    # Check if moving west is allowed (west movement constraints)
    ...
    # Border checks for moving north and south
    can_move_south = taxi_row < 4
    can_move_north = taxi_row > 0


    # 1. Change in distance to passenger, if passenger is not in the taxi
    def get_passenger_distance_change(action):
        ...
        return 0  # No change if move is out of bounds

    # 2. Change in distance to destination, if passenger is already in the taxi
    ...
    # 5. Is the action possible in each direction?
    ...
    # 6. Direction from taxi to passenger, if not already in the taxi
    ...
    # 7. Direction from taxi to destination, if passenger is in the taxi
    direction_to_dest = -1
    ...
    # 8. Previous action (to be updated later)
    previous_action = None

    should_drop_passenger = passenger_in_taxi and (taxi_row == dest_row and taxi_col == dest_col)


    # Calculate distance changes for each possible movement action
